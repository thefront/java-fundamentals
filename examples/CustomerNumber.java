...
public class CustomerNumber {
  public static void main(String[] args) {
    ...
    input = JOptionPane.showInputDialog("Enter a customer number in the form" +
                "LLLNNNN\n(LLL = Letters and NNNN = numbers)");

    // validate the input
    if (isValid(input)) {
      JOptionPane.showMessageDialog(null, "That's a a valid customer number.");
    }
    else {
      JOptionPane.showMessageDialog(null, "That is not the proper format if a " +
  ...
  /**
      The isValid method determines whether a Sting is a valid customer number.
      If so, it returns true.
      @param custNumber The String to test.
      @return true if valid, otherwise fales.
  */
